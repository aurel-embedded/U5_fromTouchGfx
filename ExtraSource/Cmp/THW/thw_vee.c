...
static void thw_drv_vee_writeNPacks(uint16_t packQty)
{
	...
	//------------------------------------------------------------------------------------
	if((thw_drv_vee_varQty <= VEE_get_variablesQty()) && (thw_drv_vee_varQty != 0)){
...
static void thw_drv_vee_veeFormat(void)
{
	...
	// Format Vee
	activePage_old = VEE_get_activePage();
	vee_error_e status = VEE_format(EE_FORCED_ERASE);
	// Compute time
	time = (HAL_GetTick() - tickStart);

	if(status != vee_error__OK){
